...
impl<H: SdioHost, D: DelayNs> SdioSdmmc<H, D> {
        ...
        match kind {
            ...
            CardKind::Mmc => {
                ...
                if let Err(_e8) = self.set_bus_width_mmc(BusWidth::Bit8) {
                    debug!("sdio: 8-bit refused ({:?}), trying 4-bit", _e8);
                    if let Err(_e4) = self.set_bus_width_mmc(BusWidth::Bit4) {
                        debug!("sdio: 4-bit refused ({:?}), staying at 1-bit", _e4);
                    }
                }
    ...
    fn wait_ready_mmc(&mut self) -> Result<OcrResponse, Error> {
        const MMC_OCR_ARG: u32 = (1 << 30) /* sector mode (HCS) */ | 0x00FF_8000;
        let mut elapsed = 0u32;
        loop {
            let cmd1 = crate::cmd::cmd1(MMC_OCR_ARG);
    ...
    /// `EXT_CSD.BUS_WIDTH`), then mirror the change down to the host.
